...
const Home = () => {
    ...
    useEffect(() => {
        ...
        if (token) {
            try {
                ...
                console.error("Token tidak valid, mengarahkan ke login.", error);
                handleLogout(); 
            }
        } else {
            navigate("/auth");
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (!prodRes.ok || !katRes.ok) throw new Error("Gagal mengambil data");
    ...
    const getBestSellerProducts = () => products.filter(p => p.stok > 10).slice(0, 6);
    const getProductsByCategory = (categoryId) => products.filter(p => p.kategoriId === categoryId).slice(0, 6);
        ...
        >
            <div className="w-full h-32 bg-white rounded-lg shadow-sm flex items-center justify-center text-amber-500 font-semibold overflow-hidden mb-3">
                {product.foto_produk ? (
                    ...
                    <div className="p-2 text-center text-sm">{product.nama_produk}</div>
                )}
            </div>
            <div>
                <h3 className="text-sm font-semibold text-gray-800 line-clamp-2">{product.nama_produk}</h3>
                <p className="text-xs text-gray-500 mt-1 line-clamp-1">{product.deskripsi || "-"}</p>
                <div className="mt-3 flex items-center justify-between">
                    <span className="text-amber-600 font-bold text-sm">{product.harga ? `Rp${(product.harga / 1000).toFixed(0)}k` : "-"}</span>
                    <span className={`text-xs font-semibold px-2 py-1 rounded-full ${product.stok > 0 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
    ...
    const HorizontalSection = ({ id, title, products: sectionProducts, icon }) => (
        <section id={id} className="bg-white rounded-2xl shadow-lg p-8 mb-8 scroll-mt-24">
            <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-gray-800">{icon} {title}</h2>
                <span className="text-sm text-gray-500">Geser ke kanan untuk lihat lebih banyak</span>
            </div>
            ...
            <div className="overflow-x-auto pb-4 flex gap-6">
                {sectionProducts && sectionProducts.length > 0 ? (
                    ...
                    <p className="text-gray-500 text-center w-full">Tidak ada produk</p>
                )}
            </div>
        </section>
    );
        ...
        <>
            ...
            <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-12 px-6">
                <div className="max-w-7xl mx-auto">
                        ...
                        <div className="relative bg-white rounded-2xl shadow-2xl overflow-hidden mb-12 h-96">
                                        ...
                                        <div className="flex items-center justify-between h-full px-8">
                                            ...
                                            <div className="w-1/2 pl-8">
                                                ...
                                                <p className="text-gray-600 text-lg mb-4 line-clamp-3">
                                                    {product.deskripsi || "Produk pilihan dari koleksi kami"}
                                                </p>
                                                <div className="flex items-center gap-4 mb-6">
                                                    <span className="text-3xl font-bold text-amber-600">
                                                        {product.harga ? `Rp${product.harga.toLocaleString()}` : "-"}
                                                    </span>
                            ...
                            <button
                                onClick={() => setCurrentCarouselIndex((prev) => 
                                    prev === 0 ? Math.min(products.length - 1, 5) : prev - 1
                                )}
                            ...
                            <button
                                onClick={() => setCurrentCarouselIndex((prev) => (prev + 1) % Math.min(products.length, 6))}
                    ...
                    {/* Main Content */}
                    {loading && (
                        <div className="text-center text-gray-500 py-12">Memuat produk...</div>
                    )}
                    {error && (
                    ...
                    {!loading && !error && (
                        <>
                            ...
                            {/* Best Seller */}
                            {getBestSellerProducts().length > 0 && (
                                <HorizontalSection 
                                    id="best-seller"
                                    title="Best Seller" 
                            ...
                            {categories.length > 0 && (
                                <>
                                    <div className="my-12 border-t-2 border-gray-200 pt-8">
                                        <h2 className="text-3xl font-bold text-gray-800 mb-8">Jelajahi Kategori</h2>
                                    </div>
